...
function scrape(o) {
    ...
    getAmount(prefix + getLink(begin, end), function(err, amount) {
        ...
        async.map(links, partial(scrapePage, prefix, out), funkit.err(partial(writeJSON, o.output, out)));
    });
}
    ...
    request(prefix + url, function(req, res, data) {
        ...
        async.map(eventLinks, function(u, cb) {
            request(u, function(req, res, data) {
                ...
                var info = $($('.tapahtumatiedot')[0]).html();
...
var parsers = {
    ...
    categories: function(d, done) {
        ...
        function fixes(a) {
            return {
                'n&auml;yttelyt': 'näyttelyt',
                'ty&ouml;pajat': 'työpajat',
                'yleis&ouml;luennot/tiedotustilaisuudet': 'yleisöluennot/tiedotustilaisuudet',
                'yleis&ouml;luennot': 'yleisöluennot',
                'jyv&auml;skyl&auml;': 'jyväskylä',
                'jyv&auml;skyl&auml; 175': 'jyväskylä 175'
            }[a] || a;
        }
    ...
    pricing: function(d, done) {
        done(null, separate(d, '<b>Hinnat ja liput:</b><br />', '<br><b>Lis&auml;tietoja:</b>'));
    },
    additionalInformation: function(d, done) {
        var p = separate(d, '<b>Lis&auml;tietoja:</b><br>').split('<br>');
