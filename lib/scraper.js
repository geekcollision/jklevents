...
function scrape(o) {
    ...
    var end = Date.tomorrow(); //today().addWeeks(1);
    ...
    zombie.visit(target, function(e, browser, status) {
        ...
        funkit.parallel(function(k, done) {
            ...
            zed.visit(k, function(e, browser, status) {
                ...
                if(info) {
                    funkit.parallel(function(o, done) {
                        var data = {
                            text: info,
                            target: k
                        };
                        o[1](data, function(err, d) {
                            done(null, [o[0], d]);
                        });
...
var parse = {
    date: function(d, done) {
        var ret = {};
        var p = parseField(d.text, 'Pvm:').split(' - ');
        var tp = parseField(d.text, 'Klo:').split(' - ');
    ...
    categories: function(d, done) {
        ...
        done(null, rtrim('; ', parseField(d.text, 'Luokat: ')).split(' ; ').
            map(funkit.lower).map(replaceNasty).map(fixes));
    },
    location: function(d, done) {
        var p = parseField(d.text, 'Paikka:');
        var west = separate(p, 'West=', '&amp;');

        if(west) console.log(d.target);
    ...
    address: function(d, done) {
        var p = parseAddress(d.text);

        done(null, p);
    },
    description: function(d, done) {
        done(null, separate(d.text, '<b>Osoite:</b>', '<b>Hinnat ja liput:</b>').split('<br />').
            slice(1).filter(funkit.id).map(partial(rtrim, ' ')).map(partial(ltrim, '\n')));
    },
    pricing: function(d, done) {
        done(null, separate(d.text, '<b>Hinnat ja liput:</b><br />', '<br /><b>Lisätietoja:</b>'));
    },
    additionalInformation: function(d, done) {
        var p = separate(d.text, '<b>Lisätietoja:</b><br />').split('<br />');
